...
var secKillObj = {
    ...
    fun: {
        ...
        getOrderResult:function(goodsId){
            $.ajax({
                url: secKillObj.url.getOrderResult(),
                type: "get",
                dataType: "json",
                data: {goodsId: goodsId},
                success: function (data) {
                    //统一错误逻辑控制,进入if表示商品信息获取失败(暂时还没有完成数据库的下单),需要Ajax轮询再次获取数据
                    if (data.code != 'OK') {
                        //延迟3000毫秒递归调用一次当前方法
                    ...
                    $("#seckillTip").html('<span style="color:red;">下单成功,共计'+orderMoney+'元&nbsp;&nbsp;&nbsp;<a href="/toPay?id='+orderId+'">立即支付</a></span>')
                },
                error: function () {
                    alert("网络异常!请稍后再试!")
                }
